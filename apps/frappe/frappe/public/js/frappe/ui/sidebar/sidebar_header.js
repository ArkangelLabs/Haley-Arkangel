frappe.ui.SidebarHeader = class SidebarHeader {
	constructor(sidebar) {
		...
		// Check if user has restricted access (Account Executive only)
		const is_restricted_user = frappe.user_roles.includes("Account Executive") &&
			!frappe.user_roles.includes("System Manager") &&
			!frappe.user_roles.includes("Administrator") &&
			!frappe.user_roles.includes("Mythril-Admin");

		this.dropdown_items = [];

		// Only show Desktop for non-restricted users
		...
		// Edit sidebar only in developer mode
		this.dropdown_items.push({
			name: "edit-sidebar",
		...
		// Only show Website for non-restricted users
		if (!is_restricted_user) {
			this.dropdown_items.push({
				name: "website",
				label: __("Website"),
